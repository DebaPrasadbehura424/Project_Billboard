...
const API_URL = "http://localhost:2000/api"; // Replace with your actual backend URL
...
function CitizenDashboard({ user }) {
  ...
  const [userName, setUserName] = useState(user?.name || "Om Prakash Lenka");
    ...
    const fetchReports = async () => {
      try {
        ...
        if (data.status) {
          ...
          // Set userName from the first report's userName (if available)
