...
export default function NavBar() {
  ...
  return (
    <nav className="fixed w-full top-0 left-0 z-50 bg-[#0A0A0A]/30 backdrop-blur-md">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button
            onClick={toggleTheme}
            className="p-2 rounded-full hover:bg-gray-800/50 transition-colors backdrop-blur-sm"
          >
            {theme === "dark" ? (
          ...
          <button
            ...
            className="md:hidden p-2 rounded-full hover:bg-gray-800/50 transition-colors backdrop-blur-sm"
          >
      ...
      {/* Mobile menu */}
      {isOpen && (
        <div className="md:hidden bg-[#0A0A0A]/80 backdrop-blur-md border-t border-gray-700/50 px-4 py-6 space-y-4">
          {navigation.map((item) => (
            <Link
              key={item.name}
              to={item.href}
